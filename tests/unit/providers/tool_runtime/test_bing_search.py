...
@pytest.fixture
def bing_search():
    return BingSearchToolRuntimeImpl(BingSearchToolConfig(api_key="test-key", top_k=3))
...
async def test_invoke_with_allowed_domains(bing_search, mock_bing_response):
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock, return_value=mock_bing_response) as mock_get:
