...
async def test_native_path_extracts_tool_calls(openai_responses_impl, mock_inference_api):
    ...
    orchestrator.enable_guardrails = False
    orchestrator.moderation_endpoint = None
    orchestrator.moderation_headers = None
...
async def test_native_path_output_guardrail_blocks_unsafe_content(openai_responses_impl, mock_inference_api):
    ...
    orchestrator.enable_guardrails = True
    orchestrator.moderation_endpoint = "https://moderation.test/v1/moderations"
    orchestrator.moderation_headers = None
    orchestrator.previous_response_id = None
    orchestrator.violation_detected = False
    orchestrator.accumulated_usage = None
    orchestrator.accumulated_builtin_output_tokens = 0
...
async def test_native_path_preserves_file_search_calls(openai_responses_impl, mock_inference_api):
    """File search tool calls in the native vLLM stream must survive into the
    ...
    fs_call = OpenAIResponseOutputMessageFileSearchToolCall(
        id="fs_1",
        queries=["Paul Graham before YC"],
        status="completed",
        results=None,
    )
    ...
    async def native_stream():
        ...
        yield OpenAIResponseObjectStreamResponseFileSearchCallCompleted(
            item_id="fs_1",
            output_index=0,
            sequence_number=3,
        )
        ...
        yield OpenAIResponseObjectStreamResponseOutputTextDelta(
            content_index=0,
            delta="Paul Graham worked at Viaweb.",
            item_id="msg_1",
            output_index=1,
            sequence_number=5,
        )
        yield OpenAIResponseObjectStreamResponseCompleted(response=response_obj, sequence_number=6)

    mock_inference_api.openai_response = AsyncMock(return_value=native_stream())

    result = await openai_responses_impl.create_openai_response(
        input="What did Paul Graham do before YC?",
        model="openai/gpt-oss-20b",
        stream=True,
    )
    chunks = [chunk async for chunk in result]
    ...
    completed = [c for c in chunks if c.type == "response.completed"]
    assert len(completed) >= 1
    final_resp = completed[-1].response
    fs_items = [o for o in final_resp.output if o.type == "file_search_call"]
    assert len(fs_items) == 1, (
        f"file_search_call dropped from final response. Got: {[o.type for o in final_resp.output]}"
    )
    assert fs_items[0].queries == ["Paul Graham before YC"]
...
def test_native_usage_accumulates_detailed_token_counters():
    """reasoning_tokens / cached_tokens must sum across native tool-loop turns,
    not be overwritten by the latest turn."""
    from ogx.providers.inline.responses.builtin.responses.streaming import StreamingResponseOrchestrator

    orch = StreamingResponseOrchestrator.__new__(StreamingResponseOrchestrator)
    orch.accumulated_usage = None
    orch.accumulated_builtin_output_tokens = 0

    def _usage(inp, out, cached, reasoning):
        return OpenAIResponseUsage(
            input_tokens=inp,
            output_tokens=out,
            total_tokens=inp + out,
            input_tokens_details=OpenAIResponseUsageInputTokensDetails(cached_tokens=cached),
            output_tokens_details=OpenAIResponseUsageOutputTokensDetails(reasoning_tokens=reasoning),
        )

    orch._accumulate_native_usage(_usage(10, 5, 1, 2))
    orch._accumulate_native_usage(_usage(20, 7, 3, 6))
