...
from ogx_api.openai_responses import OpenAIResponseOutputMessageFileSearchToolCall
...
async def test_native_path_preserves_file_search_calls(openai_responses_impl, mock_inference_api):
    ...
    async def native_stream():
        ...
        yield OpenAIResponseObjectStreamResponseOutputItemAdded(
            response_id="resp_fs", item=fs_call, output_index=0, sequence_number=2,
        )
        yield OpenAIResponseObjectStreamResponseFileSearchCallCompleted(
            item_id="fs_1", output_index=0, sequence_number=3,
        )
        yield OpenAIResponseObjectStreamResponseOutputItemDone(
            response_id="resp_fs", item=fs_call, output_index=0, sequence_number=4,
        )
        yield OpenAIResponseObjectStreamResponseOutputTextDelta(
            content_index=0, delta="Paul Graham worked at Viaweb.",
            item_id="msg_1", output_index=1, sequence_number=5,
        )
    ...
    assert len(fs_items) == 1, (
        f"file_search_call dropped from final response. Got: "
        f"{[o.type for o in final_resp.output]}"
    )
...
# --- Recording / replay of the native vLLM /v1/responses stream ---
...
def _mock_backend_stream(sse_lines: list[str]):
    ...
    def _stream(self, method, url, **kwargs):  # noqa: ANN001
        return _CM()
