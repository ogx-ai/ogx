...
def find_constraint_line(
    lines: list[str], pkg_name: str
) -> int | None:
    ...
    start, end = section
    pattern = re.compile(
        rf'^\s*"{normalize_pkg_pattern(pkg_name)}', re.IGNORECASE
    )
...
def update_constraint(
    line: str, pkg_name: str, new_version: str
) -> tuple[str, bool, str]:
    ...
    if parse_version(new_version) <= parse_version(old_version):
        return line, False, (
            f"{pkg_name}: new version {new_version} "
            f"<= current floor {old_version}"
        )
    ...
    if upper_bound_match:
        ...
        if parse_version(new_version) >= parse_version(upper_version):
            return line, False, (
                f"{pkg_name}: new version {new_version} "
                f">= upper bound <{upper_version}, skipping"
            )

    new_line = lower_bound_pattern.sub(
        lambda m: m.group(1) + new_version, line
    )
    return new_line, True, (
        f"{pkg_name}: updated >= floor from {old_version} to {new_version}"
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Update constraint-dependencies in pyproject.toml"
    )
    ...
    if line_idx is None:
        print(
            f"SKIP: {args.dependency_name} not found in constraint-dependencies"
        )
        print("updated=false")
        return 0

    new_line, changed, reason = update_constraint(
        lines[line_idx], args.dependency_name, args.dependency_version
    )
