...
**Passthrough path** (Anthropic, Ollama, vLLM): the request is forwarded directly to the provider's `/v1/messages` endpoint without any translation, since these providers natively speak the Anthropic format. This is what lets features the OpenAI format cannot represent, such as extended thinking, work with the `anthropic` provider. `config.network` (proxy, TLS, headers, timeout) applies, and errors from Anthropic keep their status code and message.
