...
says. This applies to the Responses `file_search` tool and to `vector_stores.search`.
...
- On providers that apply the weights, `score_threshold` applies to the fused RRF scores, which are at most
  `1 / (impact_factor + 1)`: about 0.016 with the default impact factor of 60. A threshold such as 0.3 therefore
  filters out every result.
- `hybrid_search` cannot be combined with `weights` or with the `weighted`, `neural`, `classifier`, or `normalized`
  rankers.
- FAISS cannot run hybrid search. FAISS vector stores ignore `hybrid_search`, log a warning and keep the requested
  search mode, which for the Responses `file_search` tool is `default_search_mode`.
- Qdrant, Weaviate, and Elasticsearch use their own hybrid search and do not apply the weights. Qdrant's hybrid search
  is a vector search limited to chunks that contain at least one query word.
