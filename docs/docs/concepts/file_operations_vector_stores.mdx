...
says. This applies to the Responses `file_search` tool and to `vector_stores.search`. Vector stores whose provider
cannot apply the weights reject the search with a `400` error rather than ranking the results some other way.
...
| `qdrant` | `400` error | Its hybrid search is a vector search limited to chunks that contain at least one query word, with no keyword ranks to weight |
| `weaviate` | `400` error | Its hybrid search hardcodes an equal (`alpha=0.5`) vector/keyword balance |
| `elasticsearch` | `400` error | Its RRF retriever drops the weights as unsupported |
