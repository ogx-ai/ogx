...
- **Thinking blocks**: `thinking` is mapped to the backend's reasoning effort for reasoning models, and the backend's reasoning text is returned as thinking blocks (see [Extended thinking](/docs/api-anthropic-messages#extended-thinking))
...
- ✅ **Extended thinking**: For reasoning models (gpt-oss, DeepSeek-R1, Qwen3, QwQ, OpenAI o-series, and similar), thinking blocks for reasoning transparency. Other models reject `thinking` requests with a 400
...
|----------|-------------------|------------------|----------------|-------|
| **OpenAI** | ❌ (translated) | ⚠️ (reasoning effort is applied; OpenAI does not return reasoning text, so there are no thinking blocks) | ❌ | Works well, no translation overhead for responses |
| **vLLM** | ✅ | ⚠️ (`thinking` is passed through; depends on the server's support) | ❌ | Serves Messages API natively with compatible models |
| **Ollama** | ✅ | ⚠️ (`thinking` is passed through; depends on the server's support) | ❌ | Serves Messages API natively with compatible models |
| **Bedrock, Fireworks, Groq, Together** | ❌ (translated) | ⚠️ (reasoning models, when the backend accepts `reasoning_effort` and returns reasoning text) | ❌ | Works via OpenAI translation |
...
|---------|------------------|-------------------------------|
| **Thinking blocks** | Native support | Reasoning models only. Effort is coarse (`budget_tokens` is not a token cap) and blocks are unsigned |
