...
| `--base-url` | `http://localhost:8321/v1` | Full OGX OpenAI-compatible API base URL, including `/v1`. Also reads `OGX_BASE_URL`. |
...
**"Failed to connect to OGX server"**: Start the OGX server first with `ogx run <config>` or point `--base-url` at a reachable server.
