...
[MultiHOP RAG](https://github.com/yixuantt/MultiHop-RAG) tests multi-hop reasoning over news articles. The system must retrieve evidence from multiple documents and synthesize an answer. Queries are sent to the Responses API with the `file_search` tool, and answers are scored by containment: whether the normalized gold answer appears as whole tokens in the reply. Containment accepts hedged replies, and most gold answers are one of a few values (Yes, No, Insufficient information, two entity names), so a constant reply listing them scores 0.8337 with no retrieval or generation. That constant-reply baseline is reported alongside every run, and a run that does not beat it is flagged. Containment is therefore only informative above 0.8337 on this dataset. Exact Match, token-level F1 and ROUGE-L are still recorded for reference.
...
| **Constant-reply baseline** | MultiHOP | Containment score of a query-independent reply listing the five most common gold answers. A run must beat it |
