...
| `native_responses` | `bool` | No | False | Use vLLM's native /v1/responses endpoint for inference instead of /v1/chat/completions. Provides proper reasoning token retention and structured token accounting. Requires vLLM with Responses API support. |
