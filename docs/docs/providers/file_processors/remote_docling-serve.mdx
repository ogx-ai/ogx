...
|-------|------|----------|---------|-------------|
| `base_url` | `str` | No | http://localhost:5001 | Base URL of the Docling Serve instance (do not include /v1 suffix) |
...
| `mode` | `str` | No | async | API mode: 'async' (use asynchronous submit/poll endpoints, recommended for both local and SaaS), 'sync' (use synchronous endpoints, fallback option), or 'auto' (detect server capabilities) |
