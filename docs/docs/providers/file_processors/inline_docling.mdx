...
| `do_ocr` | `bool` | No | True | Enable OCR for scanned documents. Set to False for digital PDFs (with embedded text) to improve processing speed by ~3x. Note: Setting to False on scanned PDFs will result in minimal text extraction. |
