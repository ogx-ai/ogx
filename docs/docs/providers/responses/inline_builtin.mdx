...
| `vector_stores_config.embedding_params` | `EmbeddingParams` | No | document_input_type=None query_input_type=None | Configuration for embedding request parameters (e.g. input_type for asymmetric models). |
| `vector_stores_config.embedding_params.document_input_type` | `str \| None` | No |  | Value for the embeddings 'input_type' parameter when embedding document chunks during ingestion. Typically 'passage' for asymmetric models. None sends no input_type. |
| `vector_stores_config.embedding_params.query_input_type` | `str \| None` | No |  | Value for the embeddings 'input_type' parameter when embedding search queries. Typically 'query' for asymmetric models. None sends no input_type. |
